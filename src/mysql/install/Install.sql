...
INSERT INTO `queryparameters_qrp` (`qrp_ID`, `qrp_qry_ID`, `qrp_Type`, `qrp_OptionSQL`, `qrp_Name`, `qrp_Description`, `qrp_Alias`, `qrp_Default`, `qrp_Required`, `qrp_InputBoxSize`, `qrp_Validation`, `qrp_NumericMax`, `qrp_NumericMin`, `qrp_AlphaMinLength`, `qrp_AlphaMaxLength`) VALUES
  ...
  (33, 18, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0),
  ...
  (305, 26, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0),
  (306, 100, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0),
  (307, 201, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0),
  (308, 300, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0),
  (309, 301, 3, 'SELECT Value, Display FROM (SELECT lst_OptionID AS Value, lst_OptionName AS Display, lst_OptionSequence AS Seq FROM list_lst WHERE lst_ID = 1 UNION ALL SELECT 0, ''Unassigned'', 9999) AS cls ORDER BY Seq, Value', 'Classification', 'Hold Ctrl (Cmd on Mac) to choose more than one. Leave empty for all classifications.', 'percls', '', 0, 0, '', 0, 0, 0, 0);
...
INSERT INTO `query_qry` (`qry_ID`, `qry_SQL`, `qry_Name`, `qry_Description`, `qry_Count`) VALUES
  ...
  (18, 'SELECT per_ID as AddToCart, per_BirthDay as Day, CONCAT(per_FirstName,'' '',per_LastName) AS Name FROM person_per WHERE per_cls_ID IN (~percls~) AND per_BirthMonth=~birthmonth~ ORDER BY per_BirthDay', 'Birthdays', 'People with birthdays in a particular month', 0),
  (22, 'SELECT per_ID as AddToCart, DAYOFMONTH(per_MembershipDate) as Day, per_MembershipDate AS DATE, CONCAT(per_FirstName,'' '',per_LastName) AS Name FROM person_per WHERE per_cls_ID IN (~percls~) AND MONTH(per_MembershipDate)=~membermonth~ ORDER BY per_MembershipDate', 'Membership anniversaries', 'People who joined in a particular month', 0),
  (25, 'SELECT per_ID as AddToCart, CONCAT(''<a href=/people/view/'',per_ID,''>'',per_FirstName,'' '',per_LastName,''</a>'') AS Name FROM person_per LEFT JOIN person2volunteeropp_p2vo ON per_id = p2vo_per_ID WHERE p2vo_vol_ID = ~volopp~ AND per_cls_ID IN (~percls~) ORDER BY per_LastName', 'Volunteers', 'Find volunteers for a particular opportunity', 1),
  (26, 'SELECT per_ID as AddToCart, CONCAT(per_FirstName,'' '',per_LastName) AS Name FROM person_per WHERE DATE_SUB(NOW(),INTERVAL ~friendmonths~ MONTH)<per_FriendDate AND per_cls_ID IN (~percls~) ORDER BY per_MembershipDate', 'Recent friends', 'Friends who signed up in previous months', 0),
  ...
  (100, 'SELECT a.per_ID as AddToCart, CONCAT(''<a href=/people/view/'',a.per_ID,''>'',a.per_FirstName,'' '',a.per_LastName,''</a>'') AS Name FROM person_per AS a LEFT JOIN person2volunteeropp_p2vo p2v1 ON (a.per_id = p2v1.p2vo_per_ID AND p2v1.p2vo_vol_ID = ~volopp1~) LEFT JOIN person2volunteeropp_p2vo p2v2 ON (a.per_id = p2v2.p2vo_per_ID AND p2v2.p2vo_vol_ID = ~volopp2~) WHERE p2v1.p2vo_per_ID=p2v2.p2vo_per_ID AND a.per_cls_ID IN (~percls~) ORDER BY per_LastName', 'Volunteers', 'Find volunteers for who match two specific opportunity codes', 1),
  (201, 'SELECT per_ID as AddToCart, CONCAT(''<a href=/people/view/'',per_ID,''>'',per_FirstName,'','',per_LastName,''</a>'') AS Name, per_LastName AS Lastname FROM person_per LEFT OUTER JOIN (SELECT event_attend.attend_id, event_attend.person_id FROM event_attend WHERE event_attend.event_id IN (~event~)) a ON person_per.per_ID = a.person_id WHERE a.attend_id is NULL AND person_per.per_cls_ID IN (~percls~) ORDER BY person_per.per_LastName, person_per.per_FirstName', 'Missing people', 'Find people who didn''t attend an event', 1),
